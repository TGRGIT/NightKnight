...
/// caches on `(kind, window, owner, maxDateMs, count, tz)` and only recomputes when one
/// of those changes. These tests pin that key's cache-hit / cache-miss semantics
...
final class AnalyticsMemoTests: XCTestCase {
    ...
    private func key(kind: AnalyticsMemo.Kind = .analytics,
                     ...
                     maxDateMs: Int64 = 1_000, count: Int = 100, tz: Int = 0) -> AnalyticsMemo.Key {
        AnalyticsMemo.Key(kind: kind, window: window, owner: owner,
                          maxDateMs: maxDateMs, count: count, tz: tz)
    }
